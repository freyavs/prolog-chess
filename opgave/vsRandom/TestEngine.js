...
function reportGameOver(chess, plLast, avgTime, x) {
  ...
  // aanpassing voor benchmarking
  //if (chess.insufficient_material() || chess.in_stalemate() ){
  //  console.log("BENCHMARK RESULT: draw");
  //}
  //else {
  //  if (plLast) {
  //    console.log("BENCHMARK RESULT: win");
  //  } else {
  //    console.log("BENCHMARK RESULT: loss");
  //  }
  //}
  //console.log("BENCHMARK AVERAGE TIME: " + avgTime/1000);
  //console.log("BENCHMARK MOVES: " + x);
...
function runTest(x, totalTime, chess) {
  if (chess.game_over()) {
    reportGameOver(chess, false, totalTime/x, x);
    return;
  }
      ...
      .then(function ({ fen, time }) {
        ...
        if (x > 5 && avgTime > 30 * 1000) {
          ...
          if (plChess.game_over()) {
            reportGameOver(plChess, true, avgTime, x);
          } else {
